#from Beta_lib import GSLIB_sgsim_2d_uncond, affine, hist, pixelplt, random_sample, locpix, locmap, regular_sample
#GSLIB2ndarray, ndarray2GSLIB, vargplt, vargplts, affine, nscore, declus, gam_2d, gamv_2d, varmapv_2d, varmap_2d, vmodel_2d, 
...
def locmap(df,xcol,ycol,vcol,xmin,xmax,ymin,ymax,vmin,vmax,title,xlabel,ylabel,vlabel,cmap):
    ...
    ixy = 0 
    plt.figure(figsize=(8,6))    
    im = plt.scatter(df[xcol],df[ycol],s=None, c=df[vcol], marker=None, cmap=cmap, norm=None, vmin=vmin, vmax=vmax, alpha=0.8, linewidths=0.8, edgecolors="black") # verts=None,
    plt.title(title)
    plt.xlim(xmin,xmax)
    plt.ylim(ymin,ymax)    
    plt.xlabel(xlabel)
...
def random_sample(array,xmin,xmax,ymin,ymax,step,nsamp,name):
    import random as rand; import numpy as np; import pandas as pd
    x = []; y = []; v = []; iix = 0; iiy = 0;
